...
class BehaviorTester {
  ...
  def testSnapshot(): Unit = {
    ...
    val out = new ListBuffer[String]()
    val l = trigger.snapshot[Int, String](b, (x: Long, y: Int) => s"$x $y").listen(out += _)
    trigger.send(100L)
    b.send(2)
    trigger.send(200L)
    b.send(9)
  ...
  def testListenOnce(): Unit = {
    ...
    val out = new ListBuffer[Int]()
    val l = Transaction { _ =>
      Operational.value(b).listen(out += _)
    }
  ...
  def testMap(): Unit = {
    ...
    val out = new ListBuffer[String]()
    val l = b.map(_.toString()).listen(out += _)
    b.send(8)
    l.unlisten()
  ...
  def testApply(): Unit = {
    ...
    val out = new ListBuffer[String]()
    val l = Behavior(bf, ba).listen(out += _)
    bf.send(b => "12 " + b)
    ba.send(6L)
    l.unlisten()
  ...
  def testLift(): Unit = {
    ...
    val out = new ListBuffer[String]()
    val l = a.lift[Long, String](b, (x, y) => s"$x $y").listen(out += _)
    a.send(12)
    b.send(6L)
    l.unlisten()
    assertEquals(List("1 5", "12 5", "12 6"), out)
  ...
  def testLiftGlitch(): Unit = {
    ...
    val a5 = a.map(x => x * 5)
    val out = new ListBuffer[String]()
    val l = a3.lift[Int, String](a5, (x, y) => s"$x $y").listen(out += _)
    a.send(2)
    l.unlisten()
  ...
  def testHoldIsDelayed(): Unit = {
    ...
    val h = e.hold(0)
    val out = new ListBuffer[String]()
    val l = e.snapshot[Int, String](h, (a: Int, b: Int) => s"$a $b").listen(out += _)
  ...
  def testLoopValueSnapshot(): Unit = {
    ...
    val l: Listener = Transaction(_ => {
      ...
      val eSnap = Operational.value(a).snapshot[String, String](b, (aa: String, bb: String) => s"$aa $bb")
  ...
  def testLiftLoop(): Unit = {
    ...
    val c = Transaction(_ => {
      val a = new BehaviorLoop[String]()
      val c_ = a.lift[String, String](b, (aa, bb) => s"$aa $bb")
