...
class StreamTester {
  ...
  def testFilterOptional(): Unit = {
    ...
    List(Some("tomato"), None, Some("peach")).foreach(e.send)
    l.unlisten()
  ...
  def testCollect(): Unit = {
    ...
    List(5, 7, 1, 2, 3).foreach(ea.send)
    l.unlisten()
