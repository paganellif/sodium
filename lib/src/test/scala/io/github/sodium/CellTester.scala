...
class CellTester {
  ...
  def testLoop(): Unit = {
    ...
    val l1 = c.listen(out1 += _)
    val l2 = c.updates().listen(out2 += _)
    s.send(5)
    s.send(7)
    l2.unlisten()
    l1.unlisten()
  ...
  def testLiftSimultaneousUpdates(): Unit = {
    ...
    val l = cellSink.lift(cell, (x: Int, y: Int) => x + y).updates().listen(out += _)
    cellSink.send(2)
    cellSink.send(7)
    l.unlisten()
