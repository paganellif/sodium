...
class Cell[A](val behavior: Behavior[A]) {
  ...
  /**
    ...
    * [[io.github.sodium.Stream.snapshot[B,C]* Stream.snapshot(Cell,(A,B)=>C)]], [[Stream.filter Stream.filter(A=>Boolean)]] and
  ...
  /**
    ...
    *
    * @see [[io.github.sodium.Stream!.holdLazy(initValue:sodium\.Lazy[A]):sodium\.Cell[A]* Stream!.holdLazy()]]
    */
  ...
  /**
    ...
    *               handler should not block. You are not allowed to use [[io.github.sodium.CellSink.send CellSink.send(A)]]
  ...
  /**
    * A variant of [[io.github.sodium.Cell.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]] that will deregister
    * the listener automatically if the listener is garbage collected. With
    * [[io.github.sodium.Cell.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]], the listener is
