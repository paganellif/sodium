...
class Behavior[A](val str: Stream[A], protected var currentValue: Option[A]) {
  ...
  /**
    ...
    * [[io.github.sodium.Stream.snapshot[B,C]* Stream.snapshot(Behavior,(A,B)=>C)]], [[Stream.filter Stream.filter(A=>Boolean)]] and
  ...
  /**
    ...
    *
    * @see [[io.github.sodium.Stream!.holdLazy(initValue:sodium\.Lazy[A]):sodium\.Behavior[A]* Stream!.holdLazy()]]
    */
  ...
  /**
    ...
    * returned [[Listener]] has a [[io.github.sodium.Listener.unlisten()* Listener.unlisten()]] method to cause the
  ...
  /**
    * A variant of [[io.github.sodium.Behavior.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]] that will deregister
    * the listener automatically if the listener is garbage collected. With
    * [[io.github.sodium.Behavior.listen(action:A=>Unit):sodium\.Listener* listen(A=>Unit)]], the listener is
    * only deregistered if [[io.github.sodium.Listener.unlisten()* Listener.unlisten()]] is called explicitly.
    */
...
object Behavior {
  ...
  def switchC[A](bba: Behavior[Behavior[A]]): Behavior[A] =
    Transaction(trans0 => {
      ...
      val l = bba.value(trans0).listen(out.node, trans0, h, suppressEarlierFirings = false)
