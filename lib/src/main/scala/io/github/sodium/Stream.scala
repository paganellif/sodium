...
class Stream[A] private (val node: Node, val finalizers: ListBuffer[Listener], val firings: ListBuffer[A]) {
  ...
  /**
    ...
    * returned [[Listener]] has a [[io.github.sodium.Listener.unlisten()* Listenerunlisten()]] method to cause the
    * listener to be removed. This is an OPERATIONAL mechanism is for interfacing between
    * the world of I/O and for FRP.
    *
    * @param handler The handler to execute when there's a new value.
    *               You should make no assumptions about what thread you are called on, and the
    *               handler should not block. You are not allowed to use [[io.github.sodium.BehaviorSink.send BehaviorSink.send(A)]]
    *               or [[io.github.sodium.StreamSink.send(a:A):Unit* StreamSink.send(A)]] in the handler.
  ...
  /**
    * A variant of [[io.github.sodium.Stream.listen(handler:A=>Unit):sodium\.Listener* listen(A=>Unit)]]  that handles the first
  ...
  final def listen_(target: Node, action: TransactionHandler[A]): Listener =
    Transaction(trans1 => listen(target, trans1, action, suppressEarlierFirings = false))
  ...
  /**
    ...
    * only deregistered if [[io.github.sodium.Listener.unlisten()* Listenerunlisten()]] is called explicitly.
    *
    * This method should be used for listeners that are to be passed to
    * [[io.github.sodium.Stream.addCleanup* Stream.addCleanup(Listener]] to ensure that things don't get kept alive
    * when they shouldn't.
    */
  ...
  def listen(target: Node,
    ...
    if (!suppressEarlierFirings && firings.nonEmpty) {
      trans.prioritized(
        target, { trans2 =>
  ...
  /**
    ...
    *          [[io.github.sodium.Behavior.sample():A* Behavior.sample()]] in which case it is equivalent to
    *          [[io.github.sodium.Stream.snapshot[B]* Stream.snapshot(Behavior)]]ing the
  ...
  /**
    ...
    * visible as the cell's current value as viewed by [[io.github.sodium.Stream.snapshot[B,C]* Stream.snapshot(Behavior,(A,B)=>C)]]
    * until the following transaction. To put this another way,
    * [[io.github.sodium.Stream.snapshot[B,C]* Stream.snapshot(Behavior,(A,B)=>C)]] always sees the value of a cell as it was before
  ...
  /**
    * A variant of [[io.github.sodium.Stream.hold(initValue:A)* hold(A)]] with an initial value captured by
  ...
  /**
    * Variant of [[io.github.sodium.Stream.snapshot[B,C]* snapshot(Behavior,(A,B)=>C)]] that captures the cell's value
  ...
  /**
    ...
    * [[io.github.sodium.Stream.hold(initValue:A)* hold(A)]] don't become visible as the cell's current value until the
    * following transaction. To put this another way, [[io.github.sodium.Stream.snapshot[B,C]* snapshot(Behavior,(A,B)=>C)]]
  ...
  /**
    * Variant of [[io.github.sodium.Stream.snapshot[B,C]* snapshot(Behavior,(A,B)=>C)]] that captures the values of
    * two cells.
    */
  ...
  /**
    ...
    *          [[io.github.sodium.Behavior.sample():A* Behavior.sample()]].
  ...
  final def coalesce(trans1: Transaction, f: (A, A) => A): Stream[A] = {
    ...
    val l = listen(out.node, trans1, h, suppressEarlierFirings = false)
    out.unsafeAddCleanup(l)
  }
  ...
  /**
    * A variant of [[io.github.sodium.Stream.collect[B,S](initState:S,f:(A,S)=>(B,S)):sodium\.Stream[B]* collect(S,(A,S)=>(B,S)]]
  ...
  /**
    * A variant of [[io.github.sodium.Stream.accum[S](initState:S,f:(A,S)=>S):sodium\.Behavior[S]* accum(S,(A,S)=>S)]] that takes an
  ...
  /**
    * Attach a listener to this stream so that its [[io.github.sodium.Listener.unlisten()* Listener.unlisten()]] is invoked
...
object Stream {
  ...
  /**
    * Variant of [[io.github.sodium.Stream.orElse(s:sodium\.Stream[A]):sodium\.Stream[A]* orElse(Stream)]]
